...
from basicsr.models.ops.dcn import DeformConvPack

from ltr.models.backbone.adain import adaptive_instance_normalization as adain
# from .adain import adaptive_instance_normalization as adain
from ltr.models.backbone.EguideF import EgF

from ltr.models.backbone.fusion_dynamic import Fusion_dynamic
...
class ALIGN(nn.Module):  # 对应对齐部分的DA

    def __init__(self, input_dim):
        """
        input:输入帧特征和事件特征连接后的特征，以及帧特征
        连接后的特征先经过二维卷积层生成偏移量
        output:偏移量和帧特征送入可变形卷积，输出对齐后的特征
        """
        ...
        self.dconv_1 = DeformConvPack(input_dim, input_dim, 3, padding=1, deformable_groups=8)
...
class ALIGN_FUSION(nn.Module):
    ...
    def forward(self, high_frame, low_frame, high_event, low_event):
        """  """
        # 使用guide_high来获得两个特征
        # fea_e1是调整后的event特征图，high_frame是基于fea_e1的注意力强化后的rgb特征图
        high_frame, fea_e1 = self.guide_high(high_frame, high_event)
        fea_e1 = fea_e1 + high_event  # 获得MA模块通道维度强化后的事件特征，和原本的事件特征相加

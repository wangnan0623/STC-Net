...
class FE108(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None):
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'eotb_train.txt')
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'eotb_val.txt')
            else:
    ...
    #     pos_path = os.path.join(seq_path, 'accumulate_events_4', '{:04}_1.jpg'.format(frame_id+1))
    #     return self.image_loader(pos_path)

    # 修改后的读取多帧事件数据。原始的读取事件图像是函数为上一个
    def _get_event(self, seq_path, frame_id):
        event_frames = []
        for i in range(1, 5):  # 4个事件帧
            event_path = os.path.join(seq_path, 'accumulate_events_4', '{:04}_{}.jpg'.format(frame_id + 1, i))
            event_img = self.image_loader(event_path)
            event_frames.append(event_img)
        return event_frames  # 返回包含4个事件的列表

...
class VisEvent(BaseVideoDataset):
    ...
    def _get_frame_path(self, seq_path, frame_id):
        img_path = os.path.join(seq_path, 'vis_imgs_jpg', '{:04}.jpg'.format(frame_id))
    ...
    #     pos_path = os.path.join(seq_path, 'event_imgs_jpg', '{:04}.jpg'.format(frame_id))
    #     return self.image_loader(pos_path)

    def _get_event(self, seq_path, frame_id):
        event_frames = []
        for i in range(1, 5):  # 4个事件帧
            event_path = os.path.join(seq_path, 'accumulate_events_4', '{:04}_{}.jpg'.format(frame_id, i))

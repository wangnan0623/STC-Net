...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        # 如果没有提供采样概率列表，则根据每个训练集的长度创建一个列表，实现均匀采样
    ...
    def __getitem__(self, index):
        ...
        data = TensorDict({'train_images': train_frames,
                           'train_events': train_events,  # 结构: [num_train_frames, 4]
                           'train_anno': train_anno['bbox'],
                           'test_images': test_frames,
                           'test_events': test_events,  # 结构: [num_train_frames, 4]

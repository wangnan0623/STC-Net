...
class KLDiMPProcessing(BaseProcessing):
    ...
    #     train_events = []
    #     test_events = []
    #     for i in range(len(data['train_events'])):
    #         train_events.append(data['train_events'][i][0])
    #         test_events.append(data['test_events'][i][0])
    #
    #
    ...
    #         data['train_events'], data['train_anno'] = self.transform['joint'](image=train_events, bbox=data['train_anno'])
    #
    #
    ...
    #         data['test_events'], data['test_anno'] = self.transform['joint'](image=test_events, bbox=data['test_anno'], new_roll=False)
    #
    #
    #     for s in ['train', 'test']:
    ...
    def __call__(self, data: TensorDict):
        ...
        ## 其中 frame1: [frame1_t1, frame1_t2, frame1_t3, frame1_t4]

        frame_num = len(data['train_events'])
        T = len(data['train_events'][0])
        ...
        if self.transform['joint'] is not None:
            ...
            for t in range(T):
                ...
                for i in range(frame_num):
                    train_events.append(data['train_events'][i][t])
                    test_events.append(data['test_events'][i][t])
                train_events_transfromed, data['train_anno'] = self.transform['joint'](image=train_events, bbox=data['train_anno'])
                test_events_transfromed, data['test_anno'] = self.transform['joint'](image=test_events, bbox=data['test_anno'], new_roll=False)
                train_transformed.append(train_events_transfromed)
                test_transformed.append(test_events_transfromed)
            data['train_events'] = train_transformed
            data['test_events'] = test_transformed
            ## 此处处理完成的事件数据，嵌套列表的层次对调了，最外层是4个元素的列表，即时间步，内层是三个元素的列表，即sampler采样的三个图像
            ## data['train_events]:
            ## list: [T1, T2, T3, T4]
            ## 其中 T1: [frame1_t1, frame2_t1, frame3_t1]
        ...
        for s in ['train', 'test']:
            ...
            event_transformed = []
            for t in range(T):
                event_data = data[s + '_events'][t]
                crops_event, _ = prutils.target_image_crop(event_data, jittered_anno, data[s + '_anno'],
                                                               self.search_area_factor, self.output_sz,
                                                               mode=self.crop_type,
                                                               max_scale_change=self.max_scale_change)
                transformed_event, _ = self.transform[s](image=crops_event, bbox=boxes, joint=False)
                event_transformed.append(transformed_event)

            data[s + '_events'] = event_transformed
        ...
        # Prepare output
        if self.mode == 'sequence':
            data = data.apply(stack_tensors)  # 这一步处理列表为张量的形式
            # train_images原本的格式为{list:3}，其中每个元素为{Tensor:(3,288,288)}
            # 处理得到的结果为{Tensor:(3,3,288,288)}，其中第一个维度对应列表中的三个元素，即采样的3个图像
            # 但stack_tensor不能处理train_events的嵌套列表，所有另外定义了一个函数来处理事件数据
            # 处理得到的事件数据格式为{Tensor:(4,3,3,288,288)}，其中第一个维度对应4个时间步，第二个维度对应采样的3个图像
            data['train_events'] = stack_events(data['train_events'])
            data['test_events'] = stack_events(data['test_events'])
        else:
            data = data.apply(lambda x: x[0] if isinstance(x, list) else x)

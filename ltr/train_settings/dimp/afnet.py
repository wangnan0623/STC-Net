...
# 设置环境变量以指定可见的GPU设备，这里选择使用第一个GPU
# os.environ['CUDA_VISIBLE_DEVICES'] = '0'

def run(settings):
    settings.description = 'Default train settings for PrDiMP with ResNet18 as backbone.'
    settings.batch_size = 32  # 默认为32
    settings.num_workers = 24  # 设置用于数据加载的工作线程数量，以加快数据读取速度
    ...
    # Train datasets
    train_dataset_path = os.path.join(settings.env.eotb_dir, 'train')
    eotb_train = EOTB(train_dataset_path)

    # Validation datasets
    test_dataset_path = os.path.join(settings.env.eotb_dir, 'test')
    eotb_test = EOTB(test_dataset_path)
    ...
    # Optimizer 创建Adam优化器
    optimizer = optim.Adam([{'params': actor.net.classifier.parameters(), 'lr': 1e-4},
                            {'params': actor.net.bb_regressor.parameters(), 'lr': 1e-4},
                            {'params': actor.net.feature_extractor.parameters(), 'lr': 1e-4},])
                            # {'params': actor.net.feature_extractor_events.parameters(), 'lr':1e-4},
                            # {'params': actor.net.align_fusion.parameters(),'lr': 1e-4}
    ...
    # 启动训练过程，设置训练epoch为52，同时支持加载最新模型和失败安全机制

...
def trackerlist(name: str, parameter_name: str, run_ids = None, netname = None, display_name: str = None):
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, run_id, netname, display_name) for run_id in run_ids]
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # FE108
        # temp_path = seq.frames[0].replace('/img/', '/accumulate_events_4/').split('.')[0]
        # VisEvent
        temp_path = seq.frames[0].replace('/vis_imgs_jpg/', '/accumulate_events_4/').split('.')[0]

        # 单个Event帧
        # event = self._read_image(temp_path + f'_1.jpg')
        # 多个Event帧
        event = []
        for i in range(1, 5):
            event_img = self._read_image(temp_path + f'_{i}.jpg')
            event.append(event_img)

        # 将event帧的路径中的accumulate_events进行替换，替换为img，以构建灰度图像的路径
        # seq_name = seq_name.replace('event_frames', 'img')
        # 对于VisEvent数据集
        # seq_name = seq_name.replace('event_imgs_jpg', 'vis_imgs_jpg')

        # idx = int(idx.split('.')[0])
        # img_path = os.path.join(seq_name, '{:04}.jpg'.format(idx))
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            # FE108
            # temp_path = frame_path.replace('/img/', '/accumulate_events_4/').split('.')[0]
            # VisEvent
            temp_path = frame_path.replace('/vis_imgs_jpg/', '/accumulate_events_4/').split('.')[0]
            ...
            # seq_name = seq_name.replace('accumulate_events', 'img')
            # # seq_name = seq_name.replace('event_imgs_jpg', 'vis_imgs_jpg')
            # idx = int(idx.split('.')[0])
            #
            # img_path = os.path.join(seq_name, '{:04}.jpg'.format(idx))

...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, events, info: dict) -> dict:
        ...
        # Convert image
        im = numpy_to_torch(image)  # Tensor:(B,C,H,W)
        # 单个事件帧
        # ev = numpy_to_torch(events)
        # 多个事件帧
        ev = []
        for event_img in events:
            event_tensor = numpy_to_torch(event_img)
            ev.append(event_tensor)
        # event = []
        # for event_img in events:
        #     event_tensor = numpy_to_torch(event_img)  # Tensor:(B,C,H,W)
        #     event.append(event_tensor)
        # event = torch.cat(event, dim=0).unsqueeze(0)  # Tensor:(B,T,C,H,W)

        # # 只取第一个事件帧
        # event_frame = events[0]
        # event = numpy_to_torch(event_frame)
        ...
        init_backbone_feat = self.generate_init_samples(im, ev)
    ...
    def track(self, image, events, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, ev, self.get_centered_sample_pos(),

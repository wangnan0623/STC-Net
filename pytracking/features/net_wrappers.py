...
class NetWithBackbone(NetWrapper):
    ...
    def extract_backbone_event(self, im: torch.Tensor):
        """Extract backbone features from the network.
        Expects a float tensor image with pixel range [0, 255]."""
        # print(f"im shape: {im.shape}, matrix shape: {matrix.shape if matrix is not None else 'None'}")
        im = self.preprocess_image(im)
        return self.net.extract_event_features(im)

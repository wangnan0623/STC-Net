...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    # Plot settings
    font_size = plot_opts.get('font_size', 15)
    ...
    # 可选：也保存 PNG 格式
    fig.savefig('{}/{}_plot.png'.format(result_plot_path, plot_type),dpi=300, format='png')
...
def plot_results(trackers, dataset, report_name, merge_results=False,
                 plot_types=('success', 'prec'), force_evaluation=False, **kwargs):
    """
...
def print_results(trackers, dataset, report_name, merge_results=False,
                  plot_types=('prec', 'success'), **kwargs):
